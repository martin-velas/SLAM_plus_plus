...
struct quaternionbase_assign_impl<Other,3,3>
{
  ...
  {
    typedef typename internal::nested_eval<Other,2>::type _TyNE;
#if defined(_MSC_VER) && !defined(__MWERKS__)
    /*const*/ _TyNE mat(const_cast<Other&>(a_mat)); // VS 2008 breaks here (const qualifier ignored with typedef)
#else
    const _TyNE mat(a_mat);
#endif
    EIGEN_USING_STD_MATH(sqrt)
