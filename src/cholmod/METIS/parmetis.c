...
**************************************************************************/
int METIS_NodeComputeSeparator(idx_t *nvtxs, idx_t *xadj, idx_t *adjncy, idx_t *vwgt, 
           idx_t *adjwgt, idx_t *options, idx_t *r_sepsize, idx_t *part) // -tHE SWINe- from METIS 4.0.1 (seems to work well with CHOLMOD)
//int METIS_ComputeVertexSeparator(idx_t *nvtxs, idx_t *xadj, idx_t *adjncy, idx_t *vwgt,
//						  idx_t *options, idx_t *r_sepsize, idx_t *part) 
{
  idx_t i, j;
  graph_t *graph;
  ctrl_t *ctrl;
  ...
  graph = SetupGraph(ctrl, *nvtxs, 1, xadj, adjncy, vwgt, NULL, adjwgt);
