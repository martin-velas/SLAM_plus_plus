...
{
	_ASSERTE(p_begin_it <= p_end_it);
	typedef typename std::iterator_traits<CForwardIterator>::value_type V;
	V v = V();//0; // more general, now can do IOTA with non-numeric types too
...
/**
 *	@brief comparison object for std::pair, comparing only the first member using less-than comparison
 */
 ...
 */
template <class CComparison>
struct CCompareFirstT {
	/**
	 *	@brief function operator; performs the comparison
	 *
	 *	@tparam CPairA is specialization of std::pair on the left
	...
	inline bool operator ()(const CPairA &a, const CPairB &b) const
	{
		return CComparison()(a.first, b.first);
	}
};

/**
 *	@brief comparison object for std::pair, comparing only the second member using less-than comparison
 */
...
struct CCompareSecondT {
	...
	inline bool operator ()(const CPairA &a, const CPairB &b) const
	{
		return CComparison()(a.second, b.second);
	}
};
