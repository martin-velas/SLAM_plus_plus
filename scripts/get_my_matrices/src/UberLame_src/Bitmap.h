...
struct TBmp {
	...
	/**
	 *	@brief samples a pixel (nearest neighbor)
	 *
	 *	@param[in] x is horizontal coordinate (can be outside of the image)
	 *	@param[in] y is horizontal coordinate (can be outside of the image)
	 *	@param[in] n_border_color is border color
	 *
	 *	@return Returns color of the specified pixel, or border color
	 *		if sampling outside of the image.
	 */
	inline uint32_t n_Sample(int x, int y, uint32_t n_border_color = 0U)
	{
		if(x >= 0 && x < n_width && y >= 0 && y < n_height)
			return p_buffer[int(x) + n_width * int(y)];
		else
			return n_border_color;
	}

	/**
	 *	@brief samples a pixel (bilinear)
	 *
	 *	@param[in] f_x is horizontal coordinate (can be outside of the image)
	 *	@param[in] f_y is horizontal coordinate (can be outside of the image)
	 *	@param[in] n_border_color is border color
	 *
	 *	@return Returns bilinearly filtered color of the specified pixel,
	 *		or border color if sampling outside of the image.
	 */
	inline uint32_t n_Sample_AA(float f_x, float f_y, uint32_t n_border_color = 0U)
	{
		int x = int(floor(f_x)), y = int(floor(f_y));
		float f_frac_x = f_x - x;
		float f_frac_y = f_y - y;
		int n_weight_00 = int(0xff * (1 - f_frac_x) * (1 - f_frac_y));
		int n_weight_10 = int(0xff * f_frac_x * (1 - f_frac_y));
		int n_weight_01 = int(0xff * (1 - f_frac_x) * f_frac_y);
		int n_weight_11 = int(0xff * f_frac_x * f_frac_y);
		return n_Modulate(n_Sample(x, y, n_border_color), n_weight_00) +
			n_Modulate(n_Sample(x + 1, y, n_border_color), n_weight_10) +
			n_Modulate(n_Sample(x, y + 1, n_border_color), n_weight_01) +
			n_Modulate(n_Sample(x + 1, y + 1, n_border_color), n_weight_11);
	}

	// todo - trilinear filter
