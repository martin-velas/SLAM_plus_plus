 ...
 *	@tparam _Ty is integer data type; this works for both signed and unsigned types
 *
 *	@param[in] n_x is value to be aligned (must be non-negative)
 *	@param[in] n_alignment is alignment (must be positive)
 *
 ...
 *	@tparam _Ty is integer data type; this works for both signed and unsigned types
 *
 *	@param[in] n_x is value to be aligned (negative values handle, rounds towards positive infinity)
 *	@param[in] n_pot_alignment is alignment, it must be power of two and positive
 *
 *	@return Returns such y, so that y >= n_x and y < n_x + n_alignment and y % n_alignment = 0.
 */
template <class _Ty>
