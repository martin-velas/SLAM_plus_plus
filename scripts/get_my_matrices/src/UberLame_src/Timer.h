 ...
 *	@date 2013-12-27
 *
 *	Added timer samplers for simple profiling / performance analysis.
 *
 */
 ...
 *
 *	@param[in] t is time in seconds to be split into hours, minutes, seconds
 */
#define PRItimeparams(t) (((t) >= 0)? "" : "-"), (int(t) / 3600), \
	((((t) >= 0)? 1 : -1) * (int(t) / 60 - 60 * (int(t) / 3600))), \
	min(float((((t) >= 0)? 1 : -1) * ((t) - 60 * (int(t) / 60))), 59.99f)

/**
 *	@def PRItimeparamsPrecise
 *	@copydoc PRItimeparams
 *	@note This version of the macro does not fix rounding of the seconds
 *		and may occasionaly display 60 in the seconds place.
 */
#define PRItimeparamsPrecise(t) (((t) >= 0)? "" : "-"), (int(t) / 3600), \
	((((t) >= 0)? 1 : -1) * (int(t) / 60 - 60 * (int(t) / 3600))), \
	((((t) >= 0)? 1 : -1) * ((t) - 60 * (int(t) / 60)))
