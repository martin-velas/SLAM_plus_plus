...
bool CTgaCodec::Get_ImageInfo(TTGAInfo &r_t_info, const char *p_s_filename)
{
	...
	size_t n_read_id_length = min(size_t(t_header.n_id_length),
...
TBmp *CTgaCodec::p_Load_TGA(const char *p_s_filename)
{
	...
	if(p_bitmap->n_former_bpc == 32 || p_bitmap->n_former_bpc == 24)
		p_bitmap->n_former_bpc = 8;
	else if(p_bitmap->n_former_bpc == 16)
		p_bitmap->n_former_bpc = 5;
	// alloc bitmap
