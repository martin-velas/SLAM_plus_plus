...
for DATASET in $ROOTS; do
	...
	for f in $FILES; do
		...
		if [[ "$COPY_ASIDE" == "1" ]]; then
			...
			( mkdir -p $name && cp "$ROOT/$f" $name/system.mtx && cp "$ROOT/$blaf" $name/system.bla && echo $name && ../scripts/blanalyze/blanalyze.sh $name/system.bla && ../bin/ba_margs_example $name $OPTION ) > ${BENCH}_${name}.log
			...
			( ../bin/ba_margs_example $name $OPTION ) >> ${BENCH}_${name}.log
			#LASTJOBID=`tail -n 1 $JOBIDFILE`
			...
			( ../bin/ba_margs_example $name $OPTION && echo ---- && rm $name/system.mtx $name/system.bla && rmdir $name ) >> ${BENCH}_${name}.log
			#rm $JOBIDFILE
			...
			( echo $name && ../scripts/blanalyze/blanalyze.sh $srcpath/system.bla && ../bin/ba_margs_example $srcpath $OPTION ) > ${BENCH}_${name}.log
			#LASTJOBID=`tail -n 1 $JOBIDFILE`
			#pbs_submit.sh -jt 10 -jn "2${name}" -nw -wf $LASTJOBID \( ../bin/ba_margs_example $srcpath $OPTION \) \>\> ${BENCH}_${name}.log | tee $JOBIDFILE
			( ../bin/ba_margs_example $srcpath $OPTION ) >> ${BENCH}_${name}.log
			#LASTJOBID=`tail -n 1 $JOBIDFILE`
			#pbs_submit.sh -jt 10 -jn "3${name}" -nw -wf $LASTJOBID \( ../bin/ba_margs_example $srcpath $OPTION \&\& echo ---- \) \>\> ${BENCH}_${name}.log
			( ../bin/ba_margs_example $srcpath $OPTION && echo ---- ) >> ${BENCH}_${name}.log
			#rm $JOBIDFILE
...
for DATASET in $ROOTS; do
	...
	../scripts/hpc2017/hpc17_parsebench.sh $LOGFILE $BENCH > ${BENCH}_log_all.csv
